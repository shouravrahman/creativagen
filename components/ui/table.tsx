...
>(({ className, ...props }, ref) => (
   <tr
      ref={ref}
      className={cn(
         "border-b border-border transition-colors hover: data-[state=selected]:bg-muted",
         className
      )}
      {...props}
   />
...
>(({ className, ...props }, ref) => (
   <td
      ref={ref}
      className={cn("p-4 border-r border-border align-middle [&:has([role=checkbox])]:pr-0", className)}
      {...props}
   />
))
